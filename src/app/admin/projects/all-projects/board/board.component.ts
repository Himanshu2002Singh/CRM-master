...
export class BoardComponent implements OnInit {
    ...
    this.projectService.getObjects().subscribe((projects: Project[]) => {
      ...
      this.lists = {
        NEWPROJECTS: projects.filter(
          (project) => project.status1 === ProjectStatus.NEWPROJECTS
        ),
        RUNNING: projects.filter(
          (project) => project.status1 === ProjectStatus.RUNNING
        ),
        ONHOLD: projects.filter(
          (project) => project.status1 === ProjectStatus.ONHOLD
        ),
        FINISHED: projects.filter(
          (project) => project.status1 === ProjectStatus.FINISHED
        ),
      };
    });
  }
  ...
  public addProject(project: Project): void {
    if (!/\S/.test(project.name)) {
      // do not add project if name is empty or contain white spaces only
      return;
    }
    this.projectService.createObject(project);
  }

  public removeProject(project: Project): void {
    // show "deleted" info
     const snack1 = this.snackBar.open("The Project has been deleted", "Undo");
